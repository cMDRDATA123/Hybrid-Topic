...
Hybrid Topic 使用语言模型发现主题并补充遗漏的内容，随后由编码器和图完成文档分配。这一分工使生成调用集中在主题发现阶段。采用精简提示词的主实验中，GPT-4.1 mini 在二十次拟合中发出55次请求，Luna 发出44次。按公开标准费率将全部输入 token 作为普通非缓存输入计算，两批生成 API 费用分别为0.3440和0.2020美元（表5）。

*表5. 生成 API 费用估算，单位为美元。每批包括四份语料各五次拟合，全部输入按标准输入费率、全部输出按标准输出费率计算，不计缓存读取折扣和缓存写入溢价；本地编码与图计算另有计算开销。*
